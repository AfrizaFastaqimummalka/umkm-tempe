...
const EMPTY_FORM = { tanggal: new Date().toISOString().split('T')[0], jumlah: '', keterangan: '', kategori: 'operasional' }

const KATEGORI_LABELS = {
  bahan_baku:  { label: 'Bahan Baku',        color: 'amber' },
  operasional: { label: 'Operasional',        color: 'gray' },
  gaji:        { label: 'Gaji Karyawan',      color: 'green' },
  listrik_air: { label: 'Listrik & Air',      color: 'gray' },
...
export default function PengeluaranPage() {
  ...
  const fetchItems = async () => {
    setLoading(true)
    try {
      const res = await api.get('/pengeluaran/', { params: { bulan: filter.bulan, tahun: filter.tahun } })
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (editItem) {
        await api.put(`/pengeluaran/${editItem.id}/`, form)
      } else {
  ...
  const totalKeluar = items.reduce((s, i) => s + Number(i.jumlah), 0)

  return (
    <div>
      <PageHeader
        title="Pengeluaran"
        subtitle={`${items.length} transaksi — Total: ${formatRupiah(totalKeluar)}`}
        action={<Button onClick={openAdd}><Plus size={16} weight="bold" /> Tambah Pengeluaran</Button>}
      />
      ...
      <div style={{ display:'flex', gap:10, marginBottom:20, flexWrap:'wrap' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <select value={filter.bulan} onChange={e => setFilter(f => ({ ...f, bulan: e.target.value }))}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <select value={filter.tahun} onChange={e => setFilter(f => ({ ...f, tahun: e.target.value }))}
            style={{ padding:'7px 10px', borderRadius:'var(--radius-sm)', border:'1.5px solid var(--gray-300)', fontSize:13, cursor:'pointer' }}>
      ...
      <Table headers={['No', 'Tanggal', 'Kategori', 'Keterangan', 'Jumlah', 'Aksi']} loading={loading}>
        {items.length === 0 && !loading ? (
          <EmptyState
            ...
            subtitle="Klik tombol Tambah Pengeluaran untuk mulai mencatat"
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Pengeluaran' : 'Tambah Pengeluaran'}>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16, marginTop: error ? 12 : 0 }}>
          ...
          <FormField label="Jumlah (Rp)" required>
            <Input type="number" name="jumlah" value={form.jumlah} onChange={handleChange} placeholder="Contoh: 75000" min="1" required />
          </FormField>
      ...
      <Modal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Hapus Pengeluaran" width={400}>
        <p style={{ fontSize:14, color:'var(--gray-600)', marginBottom:20 }}>
          Yakin ingin menghapus pengeluaran <strong>{formatRupiah(deleteConfirm?.jumlah)}</strong> pada tanggal <strong>{deleteConfirm?.tanggal}</strong>?
        </p>
